...
//            sUtils.initAllHolidays(context);
     ...
     * @param startDate
     * @param endDate
     * @return 0 表示 相等 -1表示end<start大于 1表示小于
     */
    public static int compareDateTime(DateTime startDate, DateTime endDate) {
        ...
        DateTime end = new DateTime(endDate.getYear(), endDate.getMonthOfYear(), endDate.getDayOfMonth(), 0, 0);
        if (start.getMillis() == end.getMillis()) {
            type = 0;
        } else if (start.getMillis() > end.getMillis()) {
            type = -1;
        } else {
            type = 1;
        }
    ...
    /**
     * 两个日期之前的周数总共有多少个包含自己所在的周
     *
     * @return
     */
    public static int getDefaultWeeks() {
        return getWeeksAgo(startYear, startMonth, startDay, endYear, endMonth, endDay) + 1;
    }
    ...
    /**
     * 计算开始时间和结束时间总共有多少月 包含自己
     *
     * @return
     */
    public static int getDefaultMonths() {
        return getMonthsAgo(startYear, startMonth, endYear, endMonth) + 1;
    }
